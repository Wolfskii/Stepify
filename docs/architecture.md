...
- `components/` — Svelte UI components (library sidebar uses `SidebarCollapsibleGroup`; **Finals** uses nested sidebar entries + `finals.store` / `FinalsPanel`, persisted as `finalsSessions` through `settings:get` / `settings:set`; playback queue may include timed **break** rows — `docs/practice-modes.md`)
