...
- Sidebar library: do not show per-dance track counts; indicate which dance (or All Tracks) owns the active queue with a playback-source marker (e.g. speaker), similar to Spotify. **Latin**, **Standard**, **Modes**, **Other**, and **Folders** use the same **collapsible** header pattern; **Folders** and **Other** start **collapsed**. **Finals** opens the finals builder in the main panel (`finals.store` + `FinalsPanel.svelte`); **Rounds**, **New custom mode…**, and **Other** rows remain stubs (`docs/practice-modes.md`).
